      ...
      Swal.fire({
        ...
        if(result.isConfirmed) {
          const data = { oldPassword: oldPassword, newPassword: newPassword}
            axios.put("http://localhost:4000/api/auth/changepassword", data, {
              headers: {
                accessToken: localStorage.getItem("accessToken")
              },      
            }).then((response) => {
  ...
  return (
    <div className="flex-1 min-h-screen flex flex-col justify-between">
      <div className="w-full md:p-10 p-4">
        <h2 className="text-xl md:text-2xl lg:text-3xl font-medium">Settings</h2>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-4xl'>
            <div className='bg-white p-6 rounded-xl shadow-md border'>
                ...
                <div className='flex flex-col items-center mb-6'>
                  <FaUserCircle className='md:w-14 md:h-14 h-10 w-10 text-blue-500'/>
                  <p className='text-base md:text-lg mt-6 font-medium'>{fullName}</p>
                  <p className='text-xs md:text-sm text-gray-700/90'>{username}</p>
                </div>
                <div className='space-y-2 text-gray-700 text-sm'>
                  <p><span className='font-medium text-lg'>Phone:</span> +231 779 268 242</p>
                  <p><span className='font-medium text-lg'>Location:</span> Miami, Florida</p>
                  <p><span className='font-medium text-lg'>Role:</span> Admin</p>
                </div>
            </div>
          {/* Password Update Tab */}
            <div className='bg-white p-6 rounded-xl shadow-md border'>
                ...
                <form className='space-y-4' onSubmit={handleSubmit}>
                  ...
                  <div>
                    <button type='submit'
                    className='px-8 py-2.5 rounded text-white bg-blue-500 font-medium'>
                      Update Password
                    </button>
                  </div>
                </form>
        ...
        {/** Other Information Tab */}
            <div className='col-span-1 md:col-span-2 bg-white p-6 rounded-md shadow-md border'>
